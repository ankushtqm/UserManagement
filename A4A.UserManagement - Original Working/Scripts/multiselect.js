        ...
        **/
        function Multiselect( $select, settings ) {
            ...
            this.actions = {
                ...
                $rightSelected: $(settings.rightSelected).length > 0 ? $(settings.rightSelected) : $('#' + id + '_rightSelected'),/*edited by NA added > 0**/
            ...
            events: function() {
                ...
                if ( navigator.userAgent.match(/MSIE/i)  || navigator.userAgent.indexOf('Trident/') > 0 || navigator.userAgent.indexOf('Edge/') > 0) {
                    self.$left.dblclick(function(e) {
                        //self.actions.$rightSelected.trigger('click'); /** Commented by NA **/
                        var $options = self.$left.find('option:selected'); /** Added by NA **/ 
                        if ($options.length) {
                            self.moveToRight($options, e);
                        } 
                        $(this).blur(); /** END - Added by NA **/

                    });

                    self.$right.dblclick(function(e) {
                        //self.actions.$leftSelected.trigger('click'); /** Commented by NA **/ 
                        var $options = self.$right.find('option:selected'); /** Added by NA **/ 
                        if ($options.length) {
                            self.moveToLeft($options, e);
                        } 
                        $(this).blur();  /** END - Added by NA **/
