...
{ 
    public class GroupAPIController : ApiController
    {
        ...
        {
            return !DbUtil.CheckifGroupAlreadyExists(grp);
        }
        ...
        public bool RemoveGroup(int GroupId)
        {
            ...
            bool lyrDelete = this.LyrisManager2.DeleteList(grp.LyrisListName);
            if (lyrDelete)
            {
                rtn = grp.Delete();
            }
        ...
        public HttpResponseMessage Post(ATAGroup Group)
        {
            ...
            {
                int li1 = 0;
                Group.IsNewGroup = GetIfNewGroup(Group);
                ...
                if (Group.Save())
                {
                    ...
                    try
                    {
                        if (Group.IsNewGroup)
                        {
                            ...
                            //Add current user as Manage Group Role so they can edit it in the future.
        ...
        protected void AddA4AStafftoGroups(UserGroupJsonModel ug)
        {
            ...
            ug.Save();
            if (ug.EmailAdmin || ug.BounceReports || ug.Subscribe) //Added to make sure that a A4A User is added to lyris only if a role is selected
            ...
            try
             {
                foreach (UserGroupJsonModel ug in UsersinGroup)
                {
                    ...
                    if (IsStaff)
                    {
                        AddA4AStafftoGroups(ug); 
                        //Check if committee and add a4a staff to child 
                        ATAGroup grp = new ATAGroup(ug.GroupId);
                        if (grp.IsCommittee)
                        {
                            ATAGroup childGroup = grp.GetCommitteeChildGroup();
                            ug.GroupId = childGroup.GroupId;
                            AddA4AStafftoGroups(ug);  //Add the staff to DB & Lyris for Committee information group too.
                        } 
                    }
                    else
