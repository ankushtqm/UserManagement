...
namespace A4A.UM.Controllers
{
    ...
    { 
        ...
        public Dictionary<int, string> GetAllContacts(string term,int grouptype = 0)
        {
        ...
        public Dictionary<int, string> GetAllGroups(string term, bool isMember = false, bool isActiveMB = false,int? CompanyId = null)
        {
            ...
            return DbUtil.GetAllGroupNameId(term, CompanyId, isActiveMB, isAdmin, DBUtilAPIController.CurrentUser().UserId); //.Where(x => x.Value.Contains(term)).ToDictionary(x => x.Key, x => x.Value);
        }
        ...
        ////{ 
        ////    string editorUserName = ATAMembershipUtility.Instance.ParseUsername(this.User.Identity.Name);
        ////    bool isAdmin = ATAMembershipUtility.Instance.IsCurrentUserAdmin(editorUserName);
        ////    return isAdmin;
        ////}
        //[Route("api/GetAdmin1")]
