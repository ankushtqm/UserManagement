...
namespace A4A.UM.Controllers
{
    public class DBUtilAPIController : ApiController
    {
        ...
        [Route("api/SendEmail")]
        public bool SendEmail(int userId, string body = null, string to = null, string from = null, string subject = null, bool bodyIsHtml = false)
        {
            ATAMembershipUtility.Instance.SendEmail(body, to, from, subject, bodyIsHtml, true);
            ATAMembershipProvider provider = ATAMembershipUtility.Instance.GetATAMembershipProvider();
            ATAMembershipUser currentUser = (ATAMembershipUser)provider.GetUser(provider, true);
            Transactions.setTransaction(userId, currentUser.UserId, "Username", to, "Email Sent", "Welcome Message", TransactionType.EmailSent);
            return false;
        }
